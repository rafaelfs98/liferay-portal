...
const RunFormModal: React.FC<RunFormModalProps> = ({
	modal: {modalState, observer, onClose, onSave, onSubmit},
}) => {
	const {
		formState: {errors},
		handleSubmit,
	...
	const factorOptionName = watch('factorOptionName');

	const _onSubmit = (form: Run) => {
		console.log({
			...form,
			factorOptionName: form.factorOptionName.join(' | '),
		});
	};
